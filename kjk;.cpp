...
void AllBooks(vector<Book> bf){
	...
	for(int i=0;i<size;i++){
			cout<<"+ Inserted at index" <<i<<" Book{ISBN="<<bf.at(i).getIsbn()<<" , last="<<bf.at(i).getLast()<<" , first="<<bf.at(i).getFirst()<<
				" , title="<<bf.at(i).getTitle()<<" , category="<<bf.at(i).getCategory() << " }"<<endl;

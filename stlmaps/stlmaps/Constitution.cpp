        ...
        while (i < length)
        {
            if (isalpha(line[i]))
            {
              int  temp = i;
                ...
                text = line.substr(temp,i-temp);
                for(int j=0;j<text.size() ;j++) text[j] = tolower(text[j]);
...
void make_spot_checks(WordVector& v, WordMap& m) throw(string)
{
    ...
    for (string text : texts)
    {
        ...
        /// Do the counts match?
        //if (vcount != mcount) throw(string("Count mismatch: ") + text + " " + to_string(vcount) + ":" + to_string(mcount));
        cout << endl;
    }
}
